...
func TestReadDir(t *testing.T) {
	...
	os.WriteFile(path.Join(tempDir, "FOO"), []byte(vars["FOO"].Value), 0o644)
	os.WriteFile(path.Join(tempDir, "LEN0"), []byte{}, 0o644)
	os.WriteFile(path.Join(tempDir, "EMPTY"), []byte(" \t\n"), 0o644)
	os.WriteFile(path.Join(tempDir, "BUZZ"), []byte("world\x00"), 0o644)
